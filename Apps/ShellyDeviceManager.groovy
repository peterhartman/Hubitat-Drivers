...
private void determineDeviceDriver(Map deviceStatus, String ipKey = null) {
    ...
    if (components.size() > 0) {
        ...
        if (PREBUILT_DRIVERS.containsKey(driverName)) {
            ...
            logInfo("No specialized driver for '${driverName}', using generic autoconf parent")
...
private String generateDriverName(List<String> components, Map<String, Boolean> componentPowerMonitoring = [:], Boolean isParent = false, Boolean isGen1 = false) {
    ...
    // Gen 2+ Energy Meter: em/em1 components get a dedicated EM parent
    Boolean hasEm1 = componentCounts.containsKey('em1')
    if (!isGen1 && (hasEM || hasEm1)) {
        return "${prefix} EM Parent"
    }
...
void componentSetLevel(def childDevice, Integer level, Integer transitionMs = null) {
  ...
  try {
    ...
    if (isGen1Device(childDevice)) {
      ...
      logDebug("componentSetLevel: Gen 1 light/${lightId}?turn=${params.turn}&brightness=${level}")
...
private List<Map> buildWebhookEvents(String dst, Map params) {
    ...
    switch (dst) {
        ...
        case 'smoke_alarm_off':
            events.add([name: 'smoke', value: 'clear', descriptionText: 'Smoke alarm cleared'])
            break
        case 'smoke_alarm_test':
            events.add([name: 'smoke', value: 'tested', descriptionText: 'Smoke alarm test'])
            break
        case 'smoke':  // legacy
